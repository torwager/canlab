"""Provider-agnostic tagging client for the CANlab publication database (adapted from Science of Placebo).
...
def load_prompt_parts() -> tuple[str, str]:
    """Return (system_prompt, user_template) from classify.md."""
...
def render_user_message(paper: dict, full_text: Optional[str] = None, max_fulltext_chars: int = 120_000) -> str:
    """paper: a data/papers.json record (or a candidate with the same fields)."""
    abstract = paper.get("abstract") or "NO ABSTRACT AVAILABLE"
    mode = "full_text" if full_text else ("abstract" if paper.get("abstract") else "title_only")
    parts = [
        f"Paper id: {paper.get('id', '')}",
        f"Citation: {paper.get('citation') or ''}",
        f"Title: {paper.get('title', '')}",
        f"Journal: {paper.get('journal', '')} ({paper.get('year', '')}) · kind: {paper.get('kind', 'article')} · status: {paper.get('status', 'published')}",
        f"Abstract: {abstract}",
        "",
        f"Text available: {mode}",
    ]
    if full_text:
        ft = full_text[:max_fulltext_chars]
        parts += [f"FULL TEXT (truncated to {len(ft)} characters):", ft]
    parts += ["", "Tag this paper."]
    return "\n".join(parts)
...
class Classifier:
    ...
    def _openai(self, user: str, mode: str) -> ClassifyResult:
        kwargs: dict[str, Any] = dict(
            ...
            text={"format": {"type": "json_schema", "name": "canlab_tags", "schema": self.schema, "strict": True}},
    ...
    def batch_requests(self, papers: list[dict]) -> list[dict]:
        ...
        reqs = []
        for p in papers:
            user = render_user_message(p, p.get("full_text"))
            if self.provider == "anthropic":
